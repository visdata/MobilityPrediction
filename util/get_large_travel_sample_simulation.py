...
filename = 'SIM-test-100000-30-800_20'
...
MIN_TRAVEL_NUM = 2
MIN_TRAVEL_RATIO = 0.03

stats = []
trajectory = []
filenum = 0;
	
part_trajectory = [x.rstrip() for x in open(input_dir + filename + '-trajectory') if len(x.strip())>0]
trajectory.append(part_trajectory)

part_stats = [x.rstrip().split(',') for x in open(input_dir + filename + '-sparsity')]
...
for trajectory_index in range(len(trajectory)):
	...
	travel_num = int(stats[trajectory_index][4])
