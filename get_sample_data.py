...
#     data = [(i, stats[i][target]) for i in range(len(stats))]
  ...
  while sample_index:
    ...
    while sample_num < 2 * RESAMPLE_NUM and cur_idx > 0:
      cur_idx -= 1
  ...
  for bin_index in range(1, 11):
    ...
    indices = np.random.choice(d[bin_index-1], 2 * RESAMPLE_NUM, False)
    train_set = indices[:RESAMPLE_NUM]
    test_set = indices[RESAMPLE_NUM:2*RESAMPLE_NUM]

    train_set_stat = [0, 0, 0]
    test_set_stat = [0, 0, 0]
    for i in train_set:
      train_set_stat[0] += stats[i][3]
      train_set_stat[1] += stats[i][4]
      train_set_stat[2] += stats[i][5]
    for i in test_set:
      test_set_stat[0] += stats[i][3]
      test_set_stat[1] += stats[i][4]
      test_set_stat[2] += stats[i][5]

    print('train set (%d, %d, %d, %d): %d, %d, %d' % (minute, space, target, bin_index, train_set_stat[0], train_set_stat[1], train_set_stat[2]))
    print('test set (%d, %d, %d, %d): %d, %d, %d' % (minute, space, target, bin_index, test_set_stat[0], test_set_stat[1], test_set_stat[2]))
    ...
    with open(output_dir + '-'.join(['test', str(RESAMPLE_NUM), str(minute), str(space), str(target), str(BIN_NUM), str(bin_index)]), 'w') as f:
      for i in test_set:
        for x in trajectory[i].split('|'):
          f.write(x)
          f.write('\n')
